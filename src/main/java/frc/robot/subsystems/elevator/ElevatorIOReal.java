...
public ElevatorIOReal() {
  ...
  tryUntilOk(
        ...
        5,
        () ->
        elevatorMaster.configure(elevatorConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters)
        );
  ...
  tryUntilOk(
      ...
      5,
      () ->
      elevatorSlave.configure(elevatorSlaveConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters)
      );
}
...
public void updateInputs(ElevatorIOInputs inputs) {
  ...
  inputs.velocityMetersPerSec = elevatorMaster.getEncoder().getVelocity(); //convert to meters per second
...
public void seekPosition(double position) {
  double ff = ElevatorConstants.kGReal + ElevatorConstants.kVReal * (elevatorMaster.getEncoder().getVelocity() / 60);
